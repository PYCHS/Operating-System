...
struct inode *osfs_iget(struct super_block *sb, unsigned long ino)
{
    ...
    {
        inode->i_blocks += osfs_inode->extents[i].block_count;
    }
...
int osfs_alloc_data_block(struct osfs_sb_info *sb_info, uint32_t *block_no)
{
    ...
    for (i = 0; i < sb_info->block_count; i++) {
        if (!test_bit(i, sb_info->block_bitmap)) {
            continue_amount++;
            if(continue_amount == MAX_CONTINUE_BLOCKS)
            {
                break;
            }
        }
    ...
    {
        *block_no = i;
        for(int index = i; index < i + MAX_CONTINUE_BLOCKS; index++)
        {
            set_bit(index, sb_info->block_bitmap);
        }
        sb_info->nr_free_blocks-=MAX_CONTINUE_BLOCKS;   
        return 0;
    }
...
{
    for(int i = 0; i < MAX_CONTINUE_BLOCKS; i++)
    {
